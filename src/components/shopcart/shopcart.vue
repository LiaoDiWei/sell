  ...
  <div>
   <div class="shopcart">
    <div class="content" @click="toggleList">
       ...
       <div class="content-right" v-on:click.stop="pay">
           <div class="pay" :class="{'enough': payDesc === '现在去结算','not-enough': payDesc !== '现在去结算'}">
               {{payDesc}}
           </div>
       </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    payDesc() {
      if (this.totalPrice === 0) {
        ...
        let diss = this.minPrice - this.totalPrice;
        return `还差￥${diss}起送`;
      } else if (this.totalPrice >= this.minPrice) {
        return `现在去结算`;
      }
  ...
  methods: {
    ...
    beforeEnter: function (el) {
      ...
      while (count--) {
        ...
        if (ball.show) {
          ...
          */
          //清除display样式，块元素会变成block，内联元素会变成inline
          el.style.display = '';                   
    ...
    enter: function (el, done) {
      ...
      let rf = el.offsetHeight;                   //触发浏览器重绘(vue中重绘后触发transform才有用);
      this.$nextTick(() => {
    ...
    //结算
    pay(event) {
      //event.stopPropagation();   //阻止事件冒泡,这里采用事件修饰符.stop
...
<<style scoped lang="stylus" rel="stylesheet/stylus">
  ...
  .shopcart
    ...
    .shopcart-list
      ...
      // &.fold-enter-to, &.fold-leave            //这两行可要可不要
      //   transform: translate3d(0, -100%, 0)
